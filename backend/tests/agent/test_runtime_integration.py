...
async def test_worker_uses_tools_and_persists_trace(
    ...
    eager_search = AsyncMock(side_effect=AssertionError("Eager retrieval is forbidden"))
    monkeypatch.setattr(tasks, "load_candidate_recipes", eager_search)
    monkeypatch.setattr(
        tasks, "create_plan_record", AsyncMock(return_value=SimpleNamespace(id="plan-1"))
    )
    saved = AsyncMock()
    monkeypatch.setattr(tasks, "finalize_plan_record", saved)
    fake_llm(monkeypatch, [*setup_calls(plan_data), final(plan_data)])
    result = await tasks._generate("user", days=1)
    assert result["status"] == "READY" and result["quality_status"] == "valid"
    eager_search.assert_not_awaited()
    payload = saved.call_args.kwargs["plan_data"]
    assert len(payload["generation_meta"]["days"][0]["tool_call_trace"]) == 3
