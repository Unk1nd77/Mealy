...
def validate_url_against_policy(url: str) -> tuple[bool, dict]:
    ...
    if domain_policy.recipe_url_patterns and not any(
        pattern in path for pattern in domain_policy.recipe_url_patterns
    ):
        return False, {
            "reason_codes": ["source_path_not_recipe_like"],
            "notes": [f"Source path does not match recipe patterns: {path}"],
        }
    if domain_policy.recipe_url_regexes and not any(
        re.search(pattern, path) for pattern in domain_policy.recipe_url_regexes
    ):
        return False, {
