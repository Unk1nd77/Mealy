  ...
  return (
    <>
      <Copy
        ...
        title={core.authMode === "login" ? t("onboarding.savedProfile") : t("onboarding.basics")}
      />
      <TextField
        core={core}
        field="email"
...
function BodyStep({ core }: { core: OnboardingCore }) {
  ...
  return (
    <>
      <Copy eyebrow={t("onboarding.bodyMetrics")} title={t("onboarding.cleanInputs")} />
      <div className="field-grid">
        <TextField core={core} field="age" label={t("onboarding.age")} type="number" />
        <TextField core={core} field="weight_kg" label={t("onboarding.weight")} type="number" />
        <TextField core={core} field="height_cm" label={t("onboarding.height")} type="number" />
        <label className="field">
          <span>{t("onboarding.gender")}</span>
          <select
            value={core.onboardingForm.gender}
            onChange={(event) => core.updateOnboarding("gender", event.target.value as Gender)}
          >
...
function GoalStep({ core }: { core: OnboardingCore }) {
  ...
  return (
    <>
      <Copy eyebrow={t("onboarding.goalRhythm")} title={t("onboarding.weekTone")} />
      <label className="field">
        <span>{t("onboarding.activity")}</span>
        <select
          value={core.onboardingForm.activity_level}
          onChange={(event) =>
            core.updateOnboarding("activity_level", event.target.value as ActivityLevel)
          }
        >
      ...
      <label className="field">
        ...
        <select
          ...
          onChange={(event) => core.updateOnboarding("goal", event.target.value as Goal)}
        >
      ...
      <TextArea
        ...
        placeholder="Mediterranean, high protein, fish"
      />
    </>
  )
}
...
function RestrictionsStep({ core }: { core: OnboardingCore }) {
  ...
  return (
    <>
      <Copy eyebrow={t("onboarding.restrictionsReview")} title={t("onboarding.guardrails")} />
      <TextArea
        core={core}
        field="allergies"
      ...
      <TextArea
        ...
        placeholder="gastritis, diabetes"
      />
      <div className="review-card">
        <div>
          <strong>{formatGoal(core.onboardingForm.goal)}</strong>
          <span>{formatActivity(core.onboardingForm.activity_level)}</span>
        </div>
        <div>
          <strong>
            {core.onboardingForm.weight_kg} kg / {core.onboardingForm.height_cm} cm
          </strong>
          <span>
