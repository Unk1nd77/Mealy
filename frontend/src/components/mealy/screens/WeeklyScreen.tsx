...
type WeeklyCore = Pick<
  ...
  | "user"
>
type WeeklyCommands = Pick<MealyCommands, "openCalendarExport" | "regenerateWeek">

function splitDayLabel(label: string) {
  const [weekday = label, date = ""] = label.split(",").map((part) => part.trim())
  return { weekday, date }
}
...
export function WeeklyScreen({ commands, core }: { commands: WeeklyCommands; core: WeeklyCore }) {
  const { t } = useI18n()
  if (!core.planData) return <EmptyHomeScreen commands={commands} core={core} />
  const activeDay =
    core.planData.days.find((day) => day.day_number === core.selectedDayNumber) ??
  ...
  return (
    <section className="screen-card screen-card--week">
      ...
      <div className="week-calendar" role="tablist" aria-label={t("week.daysLabel")}>
        {core.planData.days.map((day) => {
          const label = splitDayLabel(formatPlanDayLabel(core.planRecord, day.day_number))
          const isActive = day.day_number === activeDay.day_number
          const caloriesDelta = Math.round(day.total_calories - core.dailyTarget)
          ...
          return (
            ...
            >
              <span className="week-day-card__date">
                ...
                <small>{label.date || `${t("week.day")} ${day.day_number}`}</small>
              </span>
              ...
              <span className="week-day-card__delta">
                {caloriesDelta >= 0 ? "+" : ""}
                {caloriesDelta} {t("week.fromTarget")}
              </span>
              <span className="week-day-card__meals">
                {dayMealPreview(day.meals).map((meal) => (
                  <span key={`${day.day_number}-${meal.type}-${meal.recipe_id}`}>
      ...
      <div className="week-selected-day">
        ...
        <div className="week-meal-agenda">
          {activeDay.meals.map((meal) => (
            ...
            >
              <span className="week-meal-row__time">{meal.time ?? "—"}</span>
              <span className={`meal-dot meal-dot--${meal.type}`} />
              <span className="week-meal-row__main">
                <strong>{meal.title}</strong>
                <small>{formatMealType(meal.type)}</small>
              </span>
              <span className="week-meal-row__energy">{Math.round(meal.calories)} kcal</span>
            </button>
          ))}
        </div>
      </div>
