...
export function useMealyController() {
  ...
  const client = createMealyClient(state.accessToken, () => {
    setAccessToken(null);
    notice(t("notice.sessionExpired"));
  });
  ...
  function updateOnboarding<K extends keyof OnboardingForm>(
    key: K,
    value: OnboardingForm[K],
  ) {
    patch({ onboardingForm: { ...state.onboardingForm, [key]: value } });
  }
  function updateProfileDraft<K extends keyof ProfileDraft>(
    key: K,
  ...
  function getMealContext(
    ...
    for (const day of planData?.days ?? []) {
      if (dayNumber !== undefined && day.day_number !== dayNumber) continue;
      const meal = day.meals.find(
        (item) =>
          item.recipe_id === recipeId &&
          (mealType === undefined || item.type === mealType),
      );
    ...
    for (let attempt = 0; attempt < 120; attempt += 1) {
      ...
      if (status === "READY" && task.plan_id) {
        ...
        patch({
          taskId: null,
          shoppingList: null,
          globalNotice: t("notice.weekReady"),
        });
        nav.resetToScreen({ name: "home" });
        return;
      }
